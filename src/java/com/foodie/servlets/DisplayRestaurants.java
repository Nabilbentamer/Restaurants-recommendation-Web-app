 ...
 * @author hp
 */
@WebServlet(name = "DisplayRestaurants", urlPatterns = {"/DisplayRestaurants"})
public class DisplayRestaurants extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DisplayRestaurants</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DisplayRestaurants at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
                ...
                try {
                    restaus = restauDao.SelectAllRestaurantbyLocation("casablanca");
                    request.setAttribute("restaus",restaus);
                } catch (ClassNotFoundException | SQLException ex) {
                    Logger.getLogger(afterlogin.class.getName()).log(Level.SEVERE, null, ex);
                }
                
                RequestDispatcher dispatcher = request.getRequestDispatcher("ManageRestaurants.jsp");
